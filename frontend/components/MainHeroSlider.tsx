...
export default function HeroSlider({ slides, interval = 5000 }: { slides: Slide[]; interval?: number }) {
      ...
      <div className="relative h-[480px] w-full overflow-hidden rounded-2xl sm:h-[560px]">
        ...
        {slides.map((s, i) => (
          <div
            ...
            aria-hidden={i !== index}
          >
            <Image src={s.src} alt={s.title} fill priority={i === index} className="object-cover" />
            ...
            <div className="absolute bottom-6 left-6 right-6 text-white drop-shadow">
              {s.href ? (
                <Link href={s.href} className="inline-block">
                  ...
                  {s.subtitle && <p className="mt-1 text-sm opacity-85">{s.subtitle}</p>}
                </Link>
              ) : (
                <>
                  <h2 className="text-2xl font-bold md:text-3xl">{s.title}</h2>
